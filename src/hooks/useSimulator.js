...
const DEFAULTS = {
  junior:  { v0: 20, theta: 45, h0: 0 },
  senior:  { v0: 20, theta: 45, h0: 0 },
}

function makeInitialState(mode = 'junior') {
  ...
  return {
    ...
    overlays: { vectors: true, dots: false, axes: true, formulas: false, graphs: false },
    display: { numerals: mode === 'junior' ? 'bangla' : 'western' },
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'TOGGLE_NUMERALS':
      return { ...state, display: { numerals: state.display.numerals === 'bangla' ? 'western' : 'bangla' } }
    case 'SET_COMPARISON': {
      if (!action.params) return { ...state, comparison: null }
      const cp = { ...action.params }
      return { ...state, comparison: { params: cp, points: trajectoryPoints(cp.v0, cp.theta, cp.h0), results: computeResults(cp.v0, cp.theta, cp.h0) } }
    }
    case 'RESET':
